...
RTX 4070 Ti は実機検証に使用したGPUです。同じSM89アーキテクチャを持つGPUは設計上の対応範囲に含まれますが、個別の動作・性能をすべて検証しているわけではありません。


## 現在の推奨配布版

SM89 の現在の推奨配布版は **v1.0.1**（Git tag `sm89-v1.0.1`）です。

v1.0.1 は `start_vippool.bat` の文字コード互換性だけを修正したパッケージ更新です。`mona-miner.exe`、CUDAバックエンド、Stable構成、性能特性に変更はありません。

- v1.0.0: 初回公開版
- v1.0.1: `start_vippool.bat` を7-bit ASCII化し、日本語Windowsの `cmd.exe` で発生した文字化け / 誤解析を修正
...
過去の近い電力条件における実運用参考値は、旧 CUDA 10 系で約130 MH/s / 約180 Wです。この比較は同一のソースコード・同一の開発環境・完全に同じ電力条件による厳密な A/B ではありません。

## Public v1 実機検証

Public v1 SM89 は RTX 4070 Ti / Windows 11 x64 で、ビルド、正しさ確認、実行時確認、Defenderスキャン、実プール接続を含む検証を行っています。
...
```text
6A86FCF1ADDEAC8D56A762F9F2A1177CEB045D8D38A48D71CC66ACE24FB7486B
```

v1.0.0 初回公開ZIP SHA256（履歴参照）:

```text
CC4A7BB70B287EA42B294D7059C309BAEF03674833C48554ECE721226466BAAF
```

v1.0.1 ではランチャーBATのみを修正しており、`mona-miner.exe` は v1.0.0 と同一です。実行用バイナリはソースリポジトリには置かず、GitHub Releases で配布します。
...
この `sm89` ディレクトリをcurrent directoryとして、x64 Developer PowerShellから実行します。
