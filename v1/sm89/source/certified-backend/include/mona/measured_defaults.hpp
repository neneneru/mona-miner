...
// RTX 4070 Ti / SM89 Stage-5 benchmark-qualified candidate, 2026-09-16.
// Pool qualification remains a separate gate, so verified stays false here.
#if defined(MONA_TARGET_SM) && (MONA_TARGET_SM == 89) && \
    ...
    !defined(MONA_LYRA_DUMMY_TAIL) && !defined(MONA_LYRA_FULL_MASK) && \
    !defined(MONA_LYRA_STATIC_SHARED64) && !defined(MONA_LYRA_ROW3_FIRSTREAD) && \
    defined(MONA_CUBE_IMAD_PHASE_MASK) && (MONA_CUBE_IMAD_PHASE_MASK == 0)
inline MeasuredDefaults defaults_sm89() {
    constexpr Config tuned{2097152u,6u,false,256,256,256,256,256,-1};
    return {false,tuned,tuned,true};
}
#else
