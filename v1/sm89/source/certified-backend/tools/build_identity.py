...
PINNED_UPSTREAM_COMMIT = "6ff4e50987e59a70056324a94ed8667cc0bf598d"
...
def upstream_inputs(root: Path) -> tuple[str, list[dict]]:
    # Stage projects normally use a real clean Git checkout.  The integration
    # package deliberately carries an immutable source snapshot without .git,
    # so bind every file in that snapshot to the known pinned upstream commit.
    if (root / ".git").exists():
        ...
        for name in sorted(names):
            ...
            records.append(file_record(root, q))
        return commit, records
    files = [q for q in root.rglob("*") if q.is_file()]
    if not files:
        raise ValueError("pinned upstream snapshot is empty")
    records = [file_record(root, q) for q in sorted(files, key=lambda q: q.as_posix())]
    return PINNED_UPSTREAM_COMMIT, records
...
def classify_profile(settings: dict[str, str]) -> str:
    ...
    if all(settings.get(k) == v for k, v in sm89_rowout.items()):
        return "sm89-rowout-stable-v1"
    ...
    if all(settings.get(k) == v for k, v in sm89_reference.items()):
        return "sm89-stable-reference-v1"
