...
RTX 5090 は実機検証に使用したGPUです。
同じSM120アーキテクチャを持つGPUは設計上の対応範囲に含まれますが、個別の動作・性能をすべて検証しているわけではありません。
...
これらの高電力ベンチマーク値はCubeHashの採用判断に使った比較結果です。
約440 Wでの長時間実用参考値と同一の測定条件ではありません。
